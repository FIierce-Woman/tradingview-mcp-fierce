...
```bash
git clone https://github.com/fierce-woman/tradingview-mcp-fierce.git ~/tradingview-mcp-fierce
cd ~/tradingview-mcp-jackson
npm install
```

If the user specifies a different install path, use that instead of `~/tradingview-mcp-fierce`.
...
```bash
cp ~/tradingview-mcp-fierce/rules.example.json ~/tradingview-mcp-fierce/rules.json
```
...
```json
{
  "mcpServers": {
    "tradingview": {
      "command": "node",
      "args": ["/Users/YOUR_USERNAME/tradingview-mcp-fierce/src/server.js"]
    }
  }
}
```
